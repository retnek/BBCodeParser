...
class BBCodeParserTest extends PHPUnit_Framework_TestCase {
    ...
    public function testSingleParsing()
    {
        ...
        foreach ($tests as $test) {
            ...
            $this->assertEquals($result, $test['expected'], $test['in']);
        }
    }
    ...
    public function testOnlyFunctionality()
    {
        ...
        $b->only('b', 'u');
        $this->arrays_are_similar($b->getParsers(), ['img', 'url']);
    ...
    public function testCustomParser()
    {
        ...
        $b->setParser('verybold', function(ShortcodeInterface $s) {
            return '<strong>VERY '.$s->getContent().' BOLD</strong>';
        });
